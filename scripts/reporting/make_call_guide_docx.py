...
DOC.add_paragraph()
para("**本版相对 v2.2 的变化**：服务地址已迁移（见第一节，**只有一个地址**，"
     "v2.2 中的三种访问方式全部作废）；第五节的耗时与并发数字待在新机重测，"
     "暂标 [待实测]。token、字段、错误码、术语库接口均与 v2.2 一致，**贵方已实现的对接代码无需改动，"
     "只需替换服务地址**。")
...
table(["项", "说明"],
      [["原文长度", "单次上限 6000 字符，超长返回 code:422（本期不做自动切分）"],
       ["耗时（典型值）", "**[待实测]** —— v2.2 的耗时数出自旧生产机，本次迁机后不予沿用；"
                          "我方在新机重测后补发本节数字"],
       ["延迟抖动", "服务器为多服务共享资源，负载波动呈阵发性，安静时段的耗时不代表最坏情况。"
                    "**迁机后的抖动幅度同样 [待实测]**"],
       ["超时", "单请求上限 120 秒，超时返回 code:500。请将客户端超时设为 ≥120 秒，"
                "并对超时类错误重试一次"],
       ["并发", "**[待实测]** —— 新机并发能力需重测后方可给出承诺。"
      ...
      widths=[3.2, 12.8])
para("**说明**：第五节中标 [待实测] 的两项，是我方主动不沿用旧机数字所致，"
...
out = sys.argv[1] if len(sys.argv) > 1 else "标准法规翻译接口_调用说明_v3.0.docx"
DOC.save(out)
print("saved:", out)
